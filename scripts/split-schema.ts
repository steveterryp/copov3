...
type DomainCategories = {
  ...
  pov: POVDomain;
  task: DomainModels;
  activity: DomainModels;
  support: DomainModels;
};
...
function extractModelDefinition(schema: string, modelName: string): string {
...
function extractEnumDefinition(schema: string, enumName: string): string {
...
function createBaseTypes(): string {
  ...
  try {
    ...
    Object.entries(enumCategories).forEach(([category, enums]) => {
      ...
      enums.forEach(enumName => {
        ...
        if (enumDef) enumContent += enumDef + '\n\n';
      });
      ...
      if (typeof models === 'object' && !Array.isArray(models)) {
        ...
        Object.entries(models).forEach(([subdomain, subModels]) => {
          ...
          subModels.forEach(modelName => {
            ...
            if (modelDef) domainContent += modelDef + '\n\n';
          });
